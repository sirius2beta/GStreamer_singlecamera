...
typedef struct _CustomData {
	...
	GstElement *pipeline4;
	bool streaming_started;
	bool streaming_started2;
	bool streaming_started3;
	bool streaming_started4;
	int currentCameraID;
...
void on_connect(struct mosquitto *mosq, void *obj, int rc) {
	...
	mosquitto_subscribe(mosq, NULL, "USV-CMD/5200", 0);
	mosquitto_subscribe(mosq, NULL, "USV-CMD/5201", 0);
}

void on_message(struct mosquitto *mosq, void *obj, const struct mosquitto_message *msg) {
	CustomData* data = (CustomData*) obj;
	string topic((char*)msg->topic);
	...
	}else if(topic.compare(string("USV-CMD/5202")) == 0){
		cout<<"topic : 5202";
	 	if(cap.compare(string("GST")) == 0){
			string gst_command(raw_msg,space_pos+1,raw_msg.length()-space_pos-1);
			if(data->streaming_started3 == false){
				data->pipeline3 = gst_parse_launch(gst_command.c_str(), NULL);
				gst_element_set_state (data->pipeline3, GST_STATE_PLAYING);
				data->streaming_started3 = true;
			}else{
			...
			if(data->streaming_started3 == true){
				gst_element_set_state (data->pipeline3, GST_STATE_NULL);
  				gst_object_unref (data->pipeline3);
				data->streaming_started3 = false;
				cout<<"quit..."<<endl;
			}
		}else{
			cout<<"No matching cmd:"<<cap<<endl;
		}
	}else if(topic.compare(string("USV-CMD/5203")) == 0){
		cout<<"topic : 5203";
	 	if(cap.compare(string("GST")) == 0){
			string gst_command(raw_msg,space_pos+1,raw_msg.length()-space_pos-1);
			if(data->streaming_started4 == false){
				data->pipeline4 = gst_parse_launch(gst_command.c_str(), NULL);
				gst_element_set_state (data->pipeline4, GST_STATE_PLAYING);
				data->streaming_started4 = true;
			}else{
			...
			if(data->streaming_started4 == true){
				gst_element_set_state (data->pipeline4, GST_STATE_NULL);
  				gst_object_unref (data->pipeline4);
				data->streaming_started4 = false;
				cout<<"quit..."<<endl;
			}
		}else{
			cout<<"No matching cmd:"<<cap<<endl;
...
int main(int argc, char *argv[]) {
	...
	mosq = mosquitto_new("USV-CMD/USV-Alpha", true, &data);
	...
	if(data.streaming_started3){
 		gst_element_set_state (data.pipeline3, GST_STATE_NULL);
  		gst_object_unref (data.pipeline3);
	}
	if(data.streaming_started4){
 		gst_element_set_state (data.pipeline4, GST_STATE_NULL);
  		gst_object_unref (data.pipeline4);
	}
