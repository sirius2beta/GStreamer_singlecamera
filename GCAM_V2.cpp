		...
		cout<<"topic : 5201";
	 	if(cap.compare(string("START")) == 0){
			if(data->streaming_started2 == false){
				...
				data->pipeline2 = gst_parse_launch("gst-launch-1.0 -v v4l2src device=/dev/video0 num-buffers=-1 ! video/x-raw, width=640, height=480, framerate=12/1 ! videoconvert ! jpegenc ! rtpjpegpay ! udpsink host=10.8.0.4 port=5200", NULL);
				gst_element_set_state (data->pipeline2, GST_STATE_PLAYING);
			}
		}else if(cap.compare(string("GST")) == 0){
			string gst_command(raw_msg,space_pos+1,raw_msg.length()-space_pos-1);
